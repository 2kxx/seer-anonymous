...
class DiffLoss(nn.Module):
    ...
    def sample(self, z, temperature=1.0, cfg=1.0, num_timesteps=None):
        ...
        sampled_token_latent = self.gen_diffusion.p_sample_loop(
            ...
            temperature=temperature, num_timesteps=num_timesteps,
        )

        return sampled_token_latent

    def sample_refl(self, z, temperature=1.0, cfg=1.0, num_timesteps=None):
        # diffusion loss sampling
        if not cfg == 1.0:
            noise = torch.randn(z.shape[0] // 2, self.in_channels).cuda()
            noise = torch.cat([noise, noise], dim=0)
        ...
        sampled_token_latent = self.gen_diffusion.p_sample_loop_refl(
            sample_fn, noise.shape, noise, clip_denoised=False, model_kwargs=model_kwargs, progress=False,
            temperature=temperature, num_timesteps=num_timesteps,
        )
